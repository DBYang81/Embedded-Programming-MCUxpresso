...
IntegerEdit::IntegerEdit(LiquidCrystal *lcd_, LpcUart *uart_, std::string editTitle, int max, int min, float step_):
lcd(lcd_), uart(uart_), title(editTitle),upper_lim(max), lower_lim(min), step(step_){
	value = 0;
	edit  = 0;
	focus = false;
}
